...
function handler(req, res) {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    if (url.pathname === '/api/info' && req.method === 'GET')
      return json(res, 200, { ips: lanIPs(), port: PORT, scheme: HTTPS_ON ? 'https' : 'http', indexPath: '/', indexExists: fs.existsSync(INDEX), tokenRequired: !!TOKEN, rev: REV });
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  const q = '/index.html' + (TOKEN ? '?t=' + TOKEN : '');
  console.log('================================================================');
